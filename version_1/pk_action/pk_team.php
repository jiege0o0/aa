...
class Team{
	...
	function getReplayNeed(){
		...
		for($i=0;$i<$len;$i++)
		{
			$skillVO = $this->nextPKAction[$i];
			array_push($arr,$skillVO->name.'&'.$skillVO->orginOwnerID);
		}
		$oo->ac = $arr;
		// $oo->ti = $this->teamInfo;
		return $oo;
	...
	function fromReplayNeed($teamID,$oo,$baseoo,$playerArr){
		...
		for($i=0;$i<$len;$i++)
		{
			$skillStr = explode("&",$oo->ac[$i]);
			$skillStr = $skillStr[0];
			$mid = pk_skillMonster($skillStr);
			if($mid)
			{
				pk_requireSkill($mid);
			}
			$skill = pk_decodeSkill($skillStr);
			$skill->orginOwnerID = $skillStr[1];
