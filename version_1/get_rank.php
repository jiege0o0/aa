<?php 
	...
	$file  = $dataFilePath.'day_rank/server'.$serverID.'/rank'.date('Ymd', time())."_".$rankType.'.txt';//今天的排行榜数据
	...
	function getYestodayRank(){
		global $serverID;
		$index = 1;
		$file2  = $dataFilePath.'day_rank/server'.$serverID.'/rank'.date('Ymd', time() - 3600*24*$index)."_".$rankType.'.txt';//昨天的排行榜数据
